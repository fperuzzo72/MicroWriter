"""Patch em crosspoint/src/network/FirmwareFlasher.cpp: protege o slot do
editor contra o autoupdate do próprio CrossPoint.
...
o MicroWriter (o editor) está. Sem isso, um autoupdate do CrossPoint
...
cópia antiga do próprio CrossPoint), recusa a atualização antes de apagar
qualquer byte.
...
script e o equivalente em patches/crosspoint/ e patches/cpr-vcodex/
aplicam exatamente o mesmo texto.
"""

path = "crosspoint/src/network/FirmwareFlasher.cpp"
src = open(path).read()
...
h_path = "crosspoint/src/network/FirmwareFlasher.h"
