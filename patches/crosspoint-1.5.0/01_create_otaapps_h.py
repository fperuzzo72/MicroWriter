"""Cria crosspoint/src/OtaApps.h — arquivo novo, sem conflito com o CrossPoint.

Reaproveita o src/network/OtaBootSwitch.h do próprio CrossPoint, que já
contorna o bug de esp_image_verify (efuse-blk-rev) do
esp_ota_set_boot_partition nesse hardware — não precisamos reescrever essa
parte. (A versão original deste patch, embutida em build-crosspoint.yml,
chamava esp_ota_set_boot_partition() direto — o mesmo bug que a versão
para CrossInk já contorna; corrigido aqui para usar ota_boot::switchTo,
igual ao CrossInk e ao editor.)
"""
...
with open("crosspoint/src/OtaApps.h", "w") as f:
    f.write(OTA_APPS_H)
